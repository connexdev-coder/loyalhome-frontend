      ...
      <div class="h-[350px] overflow-y-scroll flex flex-col gap-2 py-2">
        <div class="flex flex-col gap-2">
          ...
          >
            ...
            <!-- Department -->
            <label v-if="props.type == 'add'" for="" class="text-md">
              {{ $t("department_name") }}
            </label>
            <div v-if="props.type == 'add'" class="relative">
              <SearchComponent
                class="w-full h-full"
                path="departments"
                @on-select="onDepartmentSelected"
                :selected-value="
...
import type { exportType, spendingType } from "~/types/tableTypes";

const boxExport = ref(false);

const selectedDepartment = ref<any>(null);
function onDepartmentSelected(newItem: any) {
  selectedDepartment.value = newItem;
...
function validateFields() {
  let requiredFields = " ";
  if (selectedDepartment.value == null && props.type == "add")
    requiredFields += " " + "department";
  if (selectedProduct.value == null && props.type == "add")
    requiredFields += " " + "product";
  if (props.manageData.getter.length == 0 && props.type == "add")
...
async function onManageDriver() {
  ...
  try {
    const response =
      props.type == "add"
        ? await useActionPost("export_invoices", {
            ...props.manageData,
        ...
        : await useActionPut(`export_invoices/${props.id}`, {
            quantity:
              selectedProduct.value.has_box && boxExport.value
                ? props.manageData.quantity * selectedProduct.value.inside_box
                : props.manageData.quantity,
            note: props.manageData.note,
          });
