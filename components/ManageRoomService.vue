    ...
    <DialogContent>
      ...
      <div class="max-h-[300px] md:max-h-[500px] overflow-y-scroll">
        ...
        >
          ...
          <input
            type="text"
            v-model="manageData.room_service_name"
          ...
          <input
            type="text"
            v-model="manageData.room_service_price"
            class="px-2 py-2 border-[1px] rounded-sm"
          />
        </form>
      </div>
...
import type { roomServiceType } from "~/types/tableTypes";
...
function validateFields() {
  let requiredFields = " ";
  if (props.manageData.room_service_name.length == 0)
    requiredFields += " " + "Name";
  if (props.manageData.room_service_price < 0) requiredFields += " " + "Price";
...
async function onManageDriver() {
  ...
  try {
    const response =
      props.type == "add"
        ? await useActionPost("room-services", {
            ...props.manageData,
          })
        : await useActionPut(`room-services/${props.id}`, {
            ...props.manageData,
          });
