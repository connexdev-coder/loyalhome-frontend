...
async function fetchPage(page: number) {
  const { data: dataData, status: dataStatus }: any = await useGet(
    `employee_offs?page=${page}&employee_id=${filterData.value.employee_id}&from=${filterData.value.from}&to=${filterData.value.to}&search=${filterData.value.search}&factory=mdf`
  );
