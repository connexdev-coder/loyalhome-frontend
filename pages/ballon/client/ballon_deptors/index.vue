  ...
  <div class="flex flex-col gap-3">
    ...
    <div class="flex flex-row items-center justify-between">
      <div class="font-bold flex flex-row items-center gap-1">
        ...
        <h1 class="text-xl uppercase">{{ $t("ballon_deptors") }}</h1>
      </div>

      <div class="flex flex-row items-center gap-1">
        <PrintDialog type="client_deptors" extra="factory=ballon" lang="ku" />
      </div>
    </div>
    ...
    >
      ...
      <template #cell-actions="{ row }">
        <div class="flex flex-row items-center justify-start gap-1">
          <NuxtLink :to="`/ballon/client/ballon_deptors/${row.client_id}`">
            <div
...
async function fetchPage(page: number) {
  const { data: dataData, status: dataStatus }: any = await useGet(
    `clients?page=${page}&id=${filterData.value.client_id}&type=owing&factory=ballon`
  );
