<template>
  <div class="flex flex-col gap-3">
    ...
    <Table
      ...
      primary_key="mdf_sale_id"
      api_route="mdf_sales"
    >
      <!-- Custom slot for 'actions' column -->
      <template #cell-actions="{ row }">
        <div class="flex flex-row items-center justify-start gap-1">
          <NuxtLink :to="`/sales/mdf/manage?id=${row.mdf_sale_id}`">
          ...
          <select
            ...
            @change="
              async (e: any) => {
                await useActionPut(`mdf_sales/${row.mdf_sale_id}`, {
...
// Define columns
const columns = [
  { key: "mdf_sale_id", label: t("invoice_number"), sortable: true },
...
async function fetchPage(page: number) {
  ...
  const { data: dataData, status: dataStatus }: any = await useGet(
    `mdf_sales?${queryParams.toString()}`
  );
