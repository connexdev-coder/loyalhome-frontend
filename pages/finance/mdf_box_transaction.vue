...
async function fetchPage(page: number) {
  const { data: dataData, status: dataStatus }: any = await useGet(
    `box_trx?page=${page}&search=${filterData.value.search}&transaction_type=${filterData.value.transaction_type}&from=${filterData.value.from}&to=${filterData.value.to}&factory=mdf`
  );
